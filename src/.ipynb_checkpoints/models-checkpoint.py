...
class CTCtopB(nn.Module):
    def __init__(self, input_size, rnn_cfg, nclasses, rnn_type='gru', expand_last_rnn=False):
        super(CTCtopB, self).__init__()

        hidden, num_layers = rnn_cfg
        #expand_last = getattr(arch_cfg, "expand_last_rnn", False)  

        if expand_last_rnn:
            # use progressive hidden sizes (last layer larger)
            if isinstance(hidden, int):
                hidden_sizes = [hidden] * (num_layers - 1) + [hidden * 2]  # if you pass 256 it will be  [256,256,512]
            elif isinstance(hidden, list):
                hidden_sizes = hidden
            else:
                raise ValueError("rnn_hidden_size must be int or list")
            ...
            for i, h in enumerate(hidden_sizes):
                if rnn_type == 'gru':
                    rnn_layer = nn.GRU(in_dim, h, num_layers=1, bidirectional=True, dropout=0.2)
                elif rnn_type == 'lstm':
                    rnn_layer = nn.LSTM(in_dim, h, num_layers=1, bidirectional=True, dropout=0.2)
                self.recs.append(rnn_layer)
                in_dim = h * 2

            self.fnl = nn.Sequential(
                nn.Dropout(0.2),
                nn.Linear(2 * hidden_sizes[-1], nclasses))
            self.cnn = nn.Sequential(nn.Dropout(0), 
    ...
    def forward(self, x):
        ...
        y = x.permute(2, 3, 0, 1)[0]
        #y = self.rec(y)[0]
            # Handle expanded vs regular RNN setup
        if hasattr(self, "recs"):  # expanded last layer version
...
class HTRNet(nn.Module):
    def __init__(self, arch_cfg, nclasses):
        ...
        if head=='cnn':
            ...
            self.top = CTCtopB(hidden, (arch_cfg.rnn_hidden_size, arch_cfg.rnn_layers), nclasses, rnn_type=arch_cfg.rnn_type, expand_last_rnn=getattr(arch_cfg, "expand_last_rnn", False))
